...
class GameSession:
    ...
    def load_and_scale_sprite(self, path):
        return pygame.transform.scale(pygame.image.load(resource_path(path)), (25, 25))
    ...
    def spawn_pokemon(self):
        hanja_data_choice = self.hanja_data[random.choice(GENS[self.current_generation]["indices"])]
        self.current_pokemon = Country(hanja_data_choice, self.font)
    ...
    def pokemon_caught(self, elapsed_time):
        ...
        # Add messages
        self.add_message(f"{self.current_pokemon.name} caught!")
    ...
    def draw_game_scores(self, screen, font):
        ...
        font_height = font.size("Caught")[1] * 1.20
        beginning = 25
        x_offset = 20
        ...
        screen.blit(self.combo_sprite, (x_offset, beginning + 1*font_height))
        self.draw_text(screen, f"Combo: {self.combo_count}", font, BLACK, 3*x_offset, 60)
        screen.blit(self.score_sprite, (x_offset, beginning + 2*font_height))
        self.draw_text(screen, f"Score: {int(self.total_score)}", font, BLACK, 3*x_offset, 100)
        screen.blit(self.mistake_sprite, (x_offset, beginning + 3*font_height))
